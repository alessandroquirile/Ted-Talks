...
def get_audio_streams(file_path, original_sample_rate, chunk_size_seconds=60):
...
def extract_long_audio_embedding(file_path, feature_extractor, audio_model, device="cpu"):
    original_sample_rate = librosa.get_samplerate(file_path)
    resample_sample_rate = feature_extractor.sampling_rate

    chunk_embeddings = []  # contains the features for each 60 seconds long audio chunk
    splitted_audio_chunks = get_audio_streams(file_path, original_sample_rate, chunk_size_seconds=60)
    for audio_chunk in splitted_audio_chunks:
        ...
        audio_chunk = torch.tensor(audio_chunk).to(device)

        # extract the featurs to feed the audio model
        extractor_data = feature_extractor(audio_chunk, sampling_rate=feature_extractor.sampling_rate, padding=True,
                                           return_tensors="pt")
